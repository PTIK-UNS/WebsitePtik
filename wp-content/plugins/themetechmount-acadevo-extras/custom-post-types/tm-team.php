...
function themetechmount_acadevo_cpt_tm_team(){

	// Getting Options
	$acadevo_theme_options = get_option('acadevo_theme_options');
	
	$team_type_title          = ( !empty($acadevo_theme_options['team_type_title']) ) ? $acadevo_theme_options['team_type_title'] : 'Team Members' ;
	...
	$team_type_slug           = ( !empty($acadevo_theme_options['team_type_slug']) ) ? $acadevo_theme_options['team_type_slug'] : 'team-member' ;
	
	$team_group_title          = ( !empty($acadevo_theme_options['team_group_title']) ) ? $acadevo_theme_options['team_group_title'] : 'Team Groups' ;
	$team_group_title_singular = ( !empty($acadevo_theme_options['team_group_title_singular']) ) ? $acadevo_theme_options['team_group_title_singular'] : 'Team Group' ;
	$team_cat_slug           = ( !empty($acadevo_theme_options['team_cat_slug']) ) ? $acadevo_theme_options['team_cat_slug'] : 'team-group' ;
	...
	add_action('do_meta_boxes', 'themetechmount_acadevo_tm_team_featured_image_box');
	function themetechmount_acadevo_tm_team_featured_image_box() {
...
add_action( 'init', 'themetechmount_acadevo_cpt_tm_team', 8 );
 ...
 *  Meta Box: Team
 */
if ( ! function_exists( 'themetechmounttmte_acadevo_tm_team_metabox_options' ) ) {
function themetechmounttmte_acadevo_tm_team_metabox_options( $options ) {
	...
	$team_extra_details_lines  = ( !empty($acadevo_theme_options['team_extra_details_lines']) ) ? $acadevo_theme_options['team_extra_details_lines'] : array() ;
	...
	$list_array = array(
		...
		array(
			"id"    => "team_details_line_btn_text",
			"type"  => "text",
			"title" => '<span class="tm-admin-team-list-icon"></span> &nbsp; '.__('Appointment Button Text', 'tmte'),
		),
		array(
			"id"    => "team_details_line_btn_link",
			"type"  => "text",
			"title" => '<span class="tm-admin-team-list-icon"></span> &nbsp; '.__('Appointment Button Link', 'tmte'),
		)
	);
	...
	if( is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		foreach( $acadevo_theme_options['team_extra_details_lines'] as $key=>$val ){
			...
			$this_array['title'] = '<span class="tm-admin-team-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['team_extra_details_line_title'], 'acadevo');
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Acadevo: %s's Details", 'tmte'), $team_type_title_singular ),
	...
	$options[]    = array(
		...
		'title'     => sprintf( esc_attr__("Acadevo: %s's Social Links", 'tmte'), $team_type_title_singular ),
		'post_type' => 'tm_team_member', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			...
			array(
				...
				'fields' => array(
					array(
						...
						'info'            => esc_attr__('Add your social services here. Also you can reorder the Social Links as per your choice. Just drag and drop items to reorder and save settings', 'acadevo'),
						'button_title'    => esc_attr__('Add New Social Links', 'acadevo'),
						'accordion_title' => 'social_icons_list_icon',
						'fields'          => array(
							array(
								'id'            => 'social_icons_list_icon',
								'type'          => 'select',
								'title'         =>  esc_attr__('Social Sevice', 'acadevo'),
								'options'  		=> array(
													...
													'rss'        => esc_attr__('RSS', 'acadevo' ),
												),
								'class'         => 'chosen',
								'default'       => 'twitter',
								'after'  		=> '<div class="cs-text-muted"><br>'.__('Select Social service from here', 'acadevo').'</div>',
							),
							array(
								'id'     		=> 'social_icons_list_link',
								'type'    		=> 'text',
								'title'   		=> esc_attr__('Link to Social service selected above', 'acadevo'),
...
add_filter( 'cs_metabox_options', 'themetechmounttmte_acadevo_tm_team_metabox_options' );
