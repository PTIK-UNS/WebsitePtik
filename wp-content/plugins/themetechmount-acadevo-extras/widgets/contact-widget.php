 ...
 * @since 1.0
 */
class acadevo_contact_widget extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'acadevo_contact_widget',
							  'description' => esc_attr__('Show Contact details with icons.', 'acadevo') );
		...
		$control_styles = array('width'   => 300,
								'height'  => 350,
								'id_base' => 'acadevo_contact_widget');
		...
		parent::__construct(
			'acadevo_contact_widget', // Base ID
			esc_attr__('Acadevo Contact Widget', 'acadevo'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Acadevo Contact Widget', 'Phone Number' . $this->id, $Phone );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Phone = icl_t( 'Acadevo Contact Widget', 'Phone Number' . $this->id, $Phone );
		}
		
		// Email
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Acadevo Contact Widget', 'Email Address' . $this->id, $Email );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Email = icl_t( 'Acadevo Contact Widget', 'Email Address' . $this->id, $Email );
		}
		
		// Website
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Acadevo Contact Widget', 'Website URL' . $this->id, $Website );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Website = icl_t( 'Acadevo Contact Widget', 'Website URL' . $this->id, $Website );
		}
		
		// Address
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Acadevo Contact Widget', 'Address' . $this->id, $Address );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Address = icl_t( 'Acadevo Contact Widget', 'Address' . $this->id, $Address );
		}
		
		// Time
		if ( function_exists( 'icl_register_string' ) ) {
			icl_register_string( 'Acadevo Contact Widget', 'Time' . $this->id, $Time );
		}
		if ( function_exists( 'icl_t' ) ) {
			$Time = icl_t( 'Acadevo Contact Widget', 'Time' . $this->id, $Time );
		}
		...
		<ul class="acadevo_contact_widget_wrapper">
			<?php if( trim($Address)!='' ): ?><li class="themetechmount-contact-address tm-acadevo-icon-location">
			<?php 
				echo wp_kses( /* HTML Filter */
					nl2br($Address),
					array(
			...
			<?php if( trim($Email)!='' ): ?><li class="themetechmount-contact-email tm-acadevo-icon-mail"><?php echo '<a href="mailto:'.sanitize_email($Email).'" target="_blank">'.sanitize_email($Email).'</a>'; ?></li><?php endif; ?>
			<?php if( trim($Phone)!='' ): ?><li class="themetechmount-contact-phonenumber tm-acadevo-icon-phone">
			<?php 
				echo wp_kses( /* HTML Filter */
					nl2br($Phone),
					array(
			...
			<?php if( trim($Website)!='' ): ?><li class="themetechmount-contact-website  tm-acadevo-icon-world"><?php echo '<a href="'.esc_url(acadevo_addhttp($Website)).'" target="_blank">'.esc_url($Website).'</a>'; ?></li><?php endif; ?>
			<?php if( trim($Time)!='' ): ?><li class="themetechmount-contact-time tm-acadevo-icon-clock">
			<?php 
				echo wp_kses( /* HTML Filter */
					nl2br($Time),
					array(
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_attr_e('Title', 'acadevo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Phone' )); ?>"><?php esc_attr_e('Phone', 'acadevo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Email' )); ?>"><?php esc_attr_e('Email', 'acadevo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Website' )); ?>"><?php esc_attr_e('Website', 'acadevo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Address' )); ?>"><?php esc_attr_e('Address', 'acadevo'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'Time' )); ?>"><?php esc_attr_e('Time', 'acadevo'); ?>:</label>
...
register_widget( 'acadevo_contact_widget' );