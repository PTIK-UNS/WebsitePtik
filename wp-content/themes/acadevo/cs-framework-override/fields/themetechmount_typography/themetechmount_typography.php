...
class CSFramework_Option_themetechmount_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
	  ...
	  echo '<small>'.esc_attr__( 'Font Family', 'acadevo' ).'</small><br>';
      ...
      echo '<optgroup label="'. esc_attr__( 'Web Safe Fonts', 'acadevo' ) .'">';
      ...
      echo '</optgroup>';
      echo '<optgroup label="'. esc_attr__( 'Google Fonts', 'acadevo' ) .'">';
	   ...
	   */
		if( ! empty( $is_backup_family ) ) {
			...
			echo '<small>'.esc_attr__( 'Backup Font Family', 'acadevo' ).'</small><br>';
      ...
      if( ! empty( $is_variant ) ) {
		...
		echo '<small>'.esc_attr__( 'Font Variants', 'acadevo' ).'</small><br>';
		...
		if( ! empty( $is_text_transform ) ) {
			...
			echo '<small>'.esc_attr__( 'Text Transform', 'acadevo' ).'</small><br>';
	   ...
	   echo '<div class="clear clr"></div>  </div> <!-- .cs-tm-typography-row2 -->';
		...
		if( ! empty( $is_font_size ) ) {
			...
			echo '<small>'.esc_attr__( 'Font Size', 'acadevo' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-font-size mini" min="1" max="200" step="1" title="'.esc_attr__( 'Font Size', 'acadevo' ).'" name="'. esc_attr($this->element_name( '[font-size]' )) .'" value="'.esc_attr($font_size_value).'">
		...
		if( ! empty( $is_line_height ) ) {
			...
			echo '<small>'.esc_attr__( 'Line Height', 'acadevo' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="1" max="200" step="1" title="'.esc_attr__( 'Line Height', 'acadevo' ).'" name="'. esc_attr($this->element_name( '[line-height]' )) .'" value="'.esc_attr($line_height_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_letter_spacing ) ) {
			echo '<div class="tm-fancy-input-w">';
			echo '<label class="cs-typography-line-height">';
			echo '<small>'.esc_attr__( 'Letter Spacing', 'acadevo' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="0" max="20" step="0.1" title="'.esc_attr__( 'Letter Spacing', 'acadevo' ).'" name="'. esc_attr($this->element_name( '[letter-spacing]' )) .'" value="'.esc_attr($letter_spacing_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_color ) ) {
			...
			echo '<small>'.esc_attr__( 'Font Color', 'acadevo' ).'</small><br>';
      ...
      if( ! empty( $is_all_varients ) ) {
		...
		echo '<div class="tm-typo-load-all-varient-text">'. esc_attr__('Load all variations of the selected font (Like bold, italic etc.)','acadevo') .'<br><small>'. esc_attr__('Loading all font variations can slow down (a little) your webpage.','acadevo').'</small></div>';
		echo '<div class="clear clr"></div>';
        echo '</label>';
		echo '</div>';
      }
      ...
      echo esc_attr__( 'Error! Can not load json file.', 'acadevo' );
