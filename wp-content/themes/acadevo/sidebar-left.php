...
if( is_page() ){
	...
	if( function_exists('is_learnpress') ){
		if (get_post_type()=='lp_course'){
			$sidebar_left = 'sidebar-left-lpcourse';
		}
	}
	...
	// The Events Calendar
	if( function_exists('is_learnpress') ){
		if ( get_post_type() == 'lp_course' || is_single('lp_course')){
			$sidebar_left = 'sidebar-left-lpcourse';
		}
	}
	...
	$sidebar_left = esc_attr($acadevo_theme_options['sidebar_post']); // Global settings
	$sidebar_left = 'sidebar-left-blog';
