...
function themetechmount_header_style_change_triggers(){
	...
	var topbar_left_text			  = '<ul class="top-contact"><li><i class="fa fa-envelope-o"></i><a href="mailto:info@yourdomainname.com">info@yourdomainname.com</a></li><li><i class="fa fa-phone"></i> +1-2345-6789-101</li></ul>';
	var titlebar_height				  = '273';
	var mainmenufont_color            = '#131d3b';
	var topbar_bg_color				  = 'darkgrey';
	var header_height                 = '100';
	var logoimg						  = 'logo.png';
	var logo_max_height				  = '42';
	var mainmenu_active_link_color    = 'skin';
	var stickymainmenufontcolor_color = '#131d3b';
	var sticky_header_bg_color        = 'white';
	var dropmenu_background_color     = '#ffffff';
	var dropdownmenufont_color        = '#666d81';
	...
	var infostack_column_two		  = '<div class="header-icon"> <div class="icon"><i class="themifyicon ti-location-pin"></i></div></div><div class="header-content"><h3>Address</h3><h5> 24 Fifth st, Los Angeles, USA</h5></div>';
	var infostack_column_three		  = '<div class="header-icon"> <div class="icon"><i class="themifyicon ti-comment-alt"></i></div></div><div class="header-content"><h3>Email</h3><h5>info@example.com</h5></div>';
	
	jQuery('input[name="acadevo_theme_options[headerstyle]"]').change(function() {
		...
		if (this.value == 'classic-highlight') { // Classic Highlight
			jQuery('select[name="acadevo_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="acadevo_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="acadevo_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('textarea[name="acadevo_theme_options[topbar_right_text]"]').val(topbar_right_text);  //
			jQuery('input[name="acadevo_theme_options[titlebar_height]"').val(titlebar_height);  // Titlebar height
			jQuery('input[name="acadevo_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="acadevo_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="acadevo_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('select[name="acadevo_theme_options[topbar_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="acadevo_theme_options[header_height]"]').val(header_height);  // Header Height
			jQuery('input[name="acadevo_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="acadevo_theme_options[logoimg][thumb-url]"]').val( imgurl+logoimg );
			jQuery('input[name="acadevo_theme_options[logoimg][full-url]').val( imgurl+logoimg );
			jQuery('input[name="acadevo_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="acadevo_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="acadevo_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('select[name="acadevo_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();		
			jQuery('input[name="acadevo_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="acadevo_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  //
				
		} else if ( this.value == 'classic-overlay' || this.value == 'overlay' ) {  // Overlay
			jQuery('select[name="acadevo_theme_options[header_bg_color]"]').val('custom').change(); 
			jQuery('input[name="acadevo_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="acadevo_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="acadevo_theme_options[show_topbar]"]').prop('checked', false);  // Show topbar
			jQuery('input[name="acadevo_theme_options[titlebar_height]"').val('365');  // Titlebar height
			jQuery('input[name="acadevo_theme_options[header_search]"]').prop('checked', false);  // Show header search
			jQuery('input[name="acadevo_theme_options[mainmenufont][color]"').iris('color', '#ffffff');   // 
			jQuery('input[name="acadevo_theme_options[header_height]"]').val('98');  // Header Height
			jQuery('input[name="acadevo_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="acadevo_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo-white.png' );
			jQuery('input[name="acadevo_theme_options[logoimg][full-url]').val( imgurl+'logo-white.png' );
			jQuery('input[name="acadevo_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="acadevo_theme_options[mainmenu_active_link_color]"]').val('custom').change(); // Select
			jQuery('input[name="acadevo_theme_options[stickymainmenufontcolor]"]').iris('color', '#ffffff');// 
			jQuery('select[name="acadevo_theme_options[sticky_header_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="acadevo_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color); // Dropdown Menu color
			jQuery('input[name="acadevo_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  

		} else if (this.value == 'infostack') {
			jQuery('select[name="acadevo_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('textarea[name="acadevo_theme_options[topbar_left_text]"]').val('<ul class="top-contact tm-highlight-left"><li>Online Education Courses Millions of people learning!</li></ul>'); 
			jQuery('textarea[name="acadevo_theme_options[topbar_right_text]"]').val('[tm-topbar-right-menu][tm-social-links tooltip="no"]'); //
			jQuery('input[name="acadevo_theme_options[titlebar_height]"').val(titlebar_height);  // Titlebar height
			jQuery('select[name="acadevo_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="acadevo_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="acadevo_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('select[name="acadevo_theme_options[topbar_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="acadevo_theme_options[header_height]"]').val('142');  // Header Height
			jQuery('input[name="acadevo_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="acadevo_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo.png' );
			jQuery('input[name="acadevo_theme_options[logoimg][full-url]').val( imgurl+'logo.png' );
			jQuery('input[name="acadevo_theme_options[logo_max_height]"]').val('47');  //
			jQuery('input[name="acadevo_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('select[name="acadevo_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="acadevo_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  //  
			jQuery('select[name="acadevo_theme_options[header_menu_bg_color]"]').val('grey').change();
			jQuery('select[name="acadevo_theme_options[sticky_header_menu_bg_color]"]').val('grey').change();
			jQuery('select[name="acadevo_theme_options[infostack_phone_text]"]').val('<a href="#">Request Info</a>').change();
			jQuery('textarea[name="acadevo_theme_options[infostack_column_one]"]').val(infostack_column_one);
			jQuery('textarea[name="acadevo_theme_options[infostack_column_two]"]').val(infostack_column_two);
			jQuery('textarea[name="acadevo_theme_options[infostack_column_three]"]').val(infostack_column_three);			
			jQuery('input[name="acadevo_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="acadevo_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
			
		} else if (this.value == 'infostack-overlay') {
			jQuery('input[name="acadevo_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="acadevo_theme_options[header_height]"]').val('150');  // Header Height
			jQuery('input[name="acadevo_theme_options[logo_max_height]"]').val('47');   //
			jQuery('input[name="acadevo_theme_options[mainmenufont][color]"').iris('color', '#ffffff');   // 
			jQuery('select[name="acadevo_theme_options[mainmenu_active_link_color]"]').val('custom').change(); // Select
			jQuery('input[name="acadevo_theme_options[show_topbar]"]').prop('checked', false);  // Show topbar
			jQuery('input[name="acadevo_theme_options[stickymainmenufontcolor]"]').iris('color', '#ffffff');// 
			jQuery('input[name="acadevo_theme_options[titlebar_height]"').val('450'); // Titlebar height
			jQuery('input[name="acadevo_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="acadevo_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo-white.png' );
			jQuery('input[name="acadevo_theme_options[logoimg][full-url]').val( imgurl+'logo-white.png' );		
			jQuery('input[name="acadevo_theme_options[dropmenu_background][color]"]').iris('color', '#ffffff');  // Dropdown Menu color
			jQuery('input[name="acadevo_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);   // Dropdown Menu color
			jQuery('select[name="acadevo_theme_options[header_menu_bg_color]"]').val('skincolor').change();
			jQuery('select[name="acadevo_theme_options[sticky_header_menu_bg_color]"]').val('skincolor').change();
			jQuery('textarea[name="acadevo_theme_options[infostack_column_one]"]').val('<ul class="top-contact"><li><i class="fa fa-envelope-o tm-skincolor"></i><a href="mailto:info@yourdomainname.com">info@yourdomainname.com</a></li><li><i class="fa fa-phone tm-skincolor"></i> +1-2345-6789-101</li></ul>');
			jQuery('textarea[name="acadevo_theme_options[infostack_column_two]"]').val('[tm-topbar-right-menu]');
			jQuery('textarea[name="acadevo_theme_options[infostack_column_three]"]').val('[tm-social-links tooltip="no"]');
			jQuery('textarea[name="acadevo_theme_options[infostack_phone_text]"]').val('<a href="#">Request Info</a>');
			jQuery('textarea[name="acadevo_theme_options[responsive_header_bg_custom_color]"]').iris('color', '#121d30'); 
		}
...
function themetechmount_show_hide_titlebar_textcolor(){
	...
	var $this      = jQuery( 'select[name="acadevo_theme_options[titlebar_text_color]"]' );
	var $page_this = jQuery( 'select[name="_themetechmount_metabox_group[titlebar_text_color]"]' );
	
	if( $this.length > 0 ){
		if( jQuery($this).val()=='custom' ){
			jQuery( 'input[name="acadevo_theme_options[titlebar_heading_font][color]"]' ).closest('.tm-typography-font-color-w').show();
			jQuery( 'input[name="acadevo_theme_options[titlebar_subheading_font][color]"]' ).closest('.tm-typography-font-color-w').show();
			jQuery( 'input[name="acadevo_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.tm-typography-font-color-w').show();
		} else {
			jQuery( 'input[name="acadevo_theme_options[titlebar_heading_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
			jQuery( 'input[name="acadevo_theme_options[titlebar_subheading_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
			jQuery( 'input[name="acadevo_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.tm-typography-font-color-w').hide();
		}
	}
...
function themetechmount_show_hide_color_picker_background(){
	...
	jQuery(items).each(function(n,val){
		...
		var $dropdown_element   = jQuery( 'select[name="acadevo_theme_options['+dropdown_name+']"]' );
		
		// show/hide the color picker on load
		if( $dropdown_element.val()=='custom' ){
			jQuery( 'input[name="acadevo_theme_options['+background_name+'][color]"]' ).closest('.tm-background-color-w').show();
		} else {
			jQuery( 'input[name="acadevo_theme_options['+background_name+'][color]"]' ).closest('.tm-background-color-w').hide();
		}
...
jQuery(document).ready( function() {
	...
	jQuery( 'select[name="acadevo_theme_options[titlebar_text_color]"]' ).change(function() {  // Theme Options
		themetechmount_show_hide_titlebar_textcolor();
	});
	...
	jQuery('input[type=radio][name="acadevo_theme_options[loaderimg]"]:checked').closest('label').addClass('tm-pre-loader-option-selected');
	jQuery('input[type=radio][name="acadevo_theme_options[loaderimg]"]').change(function() {
		jQuery('input[type=radio][name="acadevo_theme_options[loaderimg]"]').closest('label').removeClass('tm-pre-loader-option-selected');
