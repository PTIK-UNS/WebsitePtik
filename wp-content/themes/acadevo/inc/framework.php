 ...
 */
if( ! function_exists( 'acadevo_cs_framework_init' ) ) {
function acadevo_cs_framework_init(){
	// Codestar framework config
	defined('CS_OPTION'          ) or define('CS_OPTION',           'acadevo_theme_options' );
...
add_action( 'init', 'acadevo_cs_framework_init', 1 );
add_action( 'admin_init', 'acadevo_cs_framework_init', 1 );
			...
			if( isset($section['fields']) && is_array($section['fields']) ){
				foreach( $section['fields'] as $field ){
					if( isset($field['id']) && isset($field['default']) ){
						...
						$acadevo_theme_options[$field_id] = $field_val;
					}
				}
...
function themetechmount_cs_wp_enqueue_scripts($elements) {
	...
	if( count($all_fonts)>0 ){
		...
		if( !is_admin() ){
			wp_enqueue_style( 'themetechmount-cs-google-fonts', esc_url( '//fonts.googleapis.com/css?family='.$google_font_list ), array(), null );
		}
...
function themetechmount_get_font_option_values($elements){
	...
	if( is_array($elements) && count($elements)>0 ){
		...
		foreach( $elements as $element ){
			$val = ( isset($acadevo_theme_options[$element]) ) ? $acadevo_theme_options[$element] : array() ;
