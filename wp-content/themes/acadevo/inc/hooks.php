 ...
 */
if( !function_exists('themetechmount_megamenu_add_theme_acadevo_settings') ){
function themetechmount_megamenu_add_theme_acadevo_settings($themes) {
	$themes["acadevo_theme_settings"] = array(
		'title' => esc_attr__('Acadevo Theme Settings', 'acadevo'),
...
add_filter("megamenu_themes", "themetechmount_megamenu_add_theme_acadevo_settings");
...
function themetechmount_megamenu_override_default_theme($value) {
  ...
  if ( !isset($value['tm-main-menu']['theme']) ) {
    $value['tm-main-menu']['theme'] = 'acadevo_theme_settings'; // change my_custom_theme_key to the ID of your exported theme
  }
...
function themetechmount_mmmenu_breakpoint( $options ){
	...
	if( empty($megamenu_themes) ){
		$megamenu_themes = array (
			'acadevo_theme_settings' => 
				array (
				'title' => 'Acadevo Theme Settings',
				'arrow_up' => 'dash-f343',
...
function themetechmount_image_sizes(){
	...
	add_image_size( 'themetechmount-img-blog-top', $img_size_blog_top['width'], $img_size_blog_top['height'], $img_size_blog_top_crop ); // (cropped)
	...
	add_image_size( 'themetechmount-img-coursebox', 526, 470, true );
	add_image_size( 'themetechmount-img-eventbox', 1020, 680, true );
}
}
...
function themetechmount_single_skin_color(){
	...
	if( $post_id ){
		...
		if( !empty($page_meta['skincolor']) ){
			$acadevo_theme_options['skincolor'] = esc_attr($page_meta['skincolor']);
		}
	}
}
}
				...
				if( substr($meta, 0, strlen($prefix) ) == $prefix ){
					...
					if( in_array($meta_name, $all_options) ){
						if( themetechmount_is_json($value[0]) && !is_numeric($value[0]) ){
							...
							$original_val = ( isset($acadevo_theme_options[$meta_name]) ) ? $acadevo_theme_options[$meta_name] : array() ;
							$final_val    = array_merge( $original_val, $final_val );
						} else {
							// string
							$final_val = $value[0];
						}
						$acadevo_theme_options[$meta_name] = $final_val;
					}
...
/**
 * Flush out the transients used in {@see acadevo_categorized_blog()}.
 *
 * @since Acadevo 1.0
 */
if( !function_exists('acadevo_category_transient_flusher') ){
function acadevo_category_transient_flusher() {
	delete_transient( 'acadevo_categories' );
}
}
add_action( 'edit_category', 'acadevo_category_transient_flusher' );
add_action( 'save_post',     'acadevo_category_transient_flusher' );
...
function themetechmount_add_inline_dynamic_css(){
	...
	// Minify CSS style
	if( !empty( $acadevo_theme_options['minify'] ) && esc_attr($acadevo_theme_options['minify'])==true ){
		$css = themetechmount_minify_css( $css );
	}
	
	// acadevo main style
	wp_add_inline_style( 'acadevo-main-style', $css );
...
function themetechmount_single_body_background(){
	...
	if( $post_id ){
		...
		if( isset($single_meta['custom_background_switcher']) && $single_meta['custom_background_switcher']==true && !empty($single_meta['custom_background']) ){
			...
			wp_add_inline_style( 'acadevo-last-checkpoint', $css );
		}
...
if ( ! function_exists( 'acadevo_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function acadevo_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( esc_attr__( 'Continue reading %s', 'acadevo' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'acadevo_excerpt_more' );
endif;
...
function themetechmount_body_classes($bodyClass){
	...
	if( isset($acadevo_theme_options['titlebar_view']) && 
		($acadevo_theme_options['titlebar_view'] == 'default' || $acadevo_theme_options['titlebar_view'] == 'allleft' || $acadevo_theme_options['titlebar_view'] == 'allright' ) &&
		isset($acadevo_theme_options['breadcrumb_on_bottom']) &&
		$acadevo_theme_options['breadcrumb_on_bottom'] == true
	){
	...
	if( !empty($acadevo_theme_options['headerstyle']) ){
		$bodyClass[] = 'tm-headerstyle-'.esc_attr($acadevo_theme_options['headerstyle']);
	}
	
	// check if dark background set for container.
	if( isset($acadevo_theme_options['inner_background']['background-color']) && trim($acadevo_theme_options['inner_background']['background-color'])!='' && themetechmount_check_dark_color(esc_attr($acadevo_theme_options['inner_background']['background-color'])) ){
		$bodyClass[] = 'tm-dark-layout';
	}
	
	// show/hide separator line between links in dropdown menu
	if( isset($acadevo_theme_options['dropdown_menu_separator']) && trim($acadevo_theme_options['dropdown_menu_separator'])=='0' ){
		$bodyClass[] = 'tm-dropmenu-hide-sepline';
	}
	

	// Sticky Fotoer ON/OFF
	if( isset($acadevo_theme_options['stickyfooter']) && $acadevo_theme_options['stickyfooter']==true ){
		$bodyClass[] = 'themetechmount-sticky-footer';
	}
	
	// Single Portfolio
	if( is_singular('portfolio') ){
		$portfolioView        = esc_attr($acadevo_theme_options['portfolio_viewstyle']); // Global view
	...
	if( isset($acadevo_theme_options['show_topbar']) && $acadevo_theme_options['show_topbar']==true ){
	...
	// Boxed / Wide
	if( isset($acadevo_theme_options['layout']) && trim($acadevo_theme_options['layout'])!='' ){
		if( $acadevo_theme_options['layout']=='boxed' || $acadevo_theme_options['layout']=='framed' || $acadevo_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'themetechmount-boxed';
		}
		if( $acadevo_theme_options['layout']=='framed' || $acadevo_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'themetechmount-boxed-'.sanitize_html_class($acadevo_theme_options['layout']);
		}
		
		$bodyClass[] = sanitize_html_class( 'themetechmount-'.trim($acadevo_theme_options['layout']));
		if( $acadevo_theme_options['layout']=='fullwide' ){
			if( isset($acadevo_theme_options['full_wide_elements']['content']) && $acadevo_theme_options['full_wide_elements']['content']=='1' ){
	...
	if( isset($acadevo_theme_options['logoimg_retina']['url']) && $acadevo_theme_options['logoimg_retina']['url']!=''){
	...
	$headerstyle_global = '';
	$headerstyle_page   = '';
	if( isset($acadevo_theme_options['headerstyle']) && trim($acadevo_theme_options['headerstyle'])!='' ){
		$headerstyle_global = sanitize_html_class($acadevo_theme_options['headerstyle']);
	}
	if( is_page() ){
	...
	// One Page website
	if( isset($acadevo_theme_options['one_page_site']) && $acadevo_theme_options['one_page_site']==true ){
...
// Course Single
add_action( 'learn-press/content-landing-summary', 'themetechmount_course_descbox' );

function themetechmount_course_descbox(){
	...
	if ( function_exists( 'learn_press_get_course_rate' ) ) {
		...
		$course_totalrev     = $course_rate_res['total'];	
	}
?>
<div class="tm-single-course-box">
<ul class="tm-coursedetails-box">
   <li class="tm-meta-line tm-author"> 
    <?php echo get_avatar( $author_id, 130 ); ?>
     <strong><?php echo esc_attr__( 'Instructor', 'acadevo' ); ?></strong> 	
      <strong><?php echo themetechmount_wp_kses($instructor); ?></strong>
   </li>
    <li class="tm-meta-line tm-enroll-stu"> 
	...
	<?php endif; ?>
     <div class="tm-stu-inroll"><strong><?php echo esc_attr($students); ?> <?php echo esc_attr__( 'Students Enrolled', 'acadevo' ); ?></strong></div>
   </li>
	<li class="tm-meta-line tm-course-dur"> 
     <strong><?php echo esc_attr__( 'Course Duration', 'acadevo' ); ?></strong> 
      <?php echo esc_attr($duration); ?>
   </li>

    <li class="tm-meta-line tm-course-price"> 
	 <strong><?php echo esc_attr__( 'Price', 'acadevo' ); ?></strong> 
     <?php echo esc_attr($course_price); ?>
   </li>
	<?php if ( ! $user->has_enrolled_course( $course->get_id() ) ) { ?>
		<li class="tm-meta-line tm-course-button"> <?php learn_press_course_buttons();?></li>
	<?php } ?>
  </ul>
</div>
<?php		
...
add_action( 'show_user_profile', 'tm_instructor_user_profile_fields' );
add_action( 'edit_user_profile', 'tm_instructor_user_profile_fields' );

function tm_instructor_user_profile_fields( $user ) {
	...
	<table class="form-table">
		<tbody>
		 <tr>
			<th><label for="designation"><?php esc_attr_e( 'Designation', 'acadevo' ); ?></label></th>
			<td>
				<input type="text" name="tm_instructor_designation" id="tm_instructor_designation" value="<?php echo esc_attr( get_the_author_meta( 'tm_instructor_designation', $user->ID ) ); ?>" class="regular-text" />
			</td>
		</tr>
...
add_action( 'personal_options_update', 'tm_instructor_user_profile_fields_update' );
add_action( 'edit_user_profile_update', 'tm_instructor_user_profile_fields_update' );


function tm_instructor_user_profile_fields_update( $user_id=false ) {
	if ( !$user_id ) {
	...
	$meta = $_POST['tm_instructor_designation'];
	
	update_user_meta( $user_id, 'tm_instructor_designation', $meta );
}


//Profile

add_filter( 'learn-press/profile-settings-fields/avatar', 'tm_instructor_change_avatar_size' ); // change avatar size

function tm_instructor_change_avatar_size( $args ){
	foreach ( $args as $key => $value ) {
		if ( isset($value['id']) && $value['id'] == 'profile_picture_thumbnail_size' ) {
			$args[$key]['default'] = array( 600, 700, 'yes' );
			$args[$key]['desc'] = '';
		}
	}
	return $args;
 ...
 */
function themetechmount_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'themetechmount_pingback_header' );