...
if( function_exists('is_woocommerce') ){  /* Check if WooCommerce plugin activated */
	...
	function tm_wc_loop_buttons(){
		...
		echo '<div class="tm-shop-icon">';
			...
			if( shortcode_exists('yith_wcwl_add_to_wishlist') || class_exists( 'YITH_WCQV_Frontend' ) ) {	
			
				echo '<div class="tm-hover-icons">';
				// YITH Wishlist
				if( shortcode_exists('yith_wcwl_add_to_wishlist') ) {
					echo do_shortcode('[yith_wcwl_add_to_wishlist]');
				}
				if( class_exists('YITH_Woocompare_Frontend') ){
				$YITH_Woocompare_Frontend = new YITH_Woocompare_Frontend; echo esc_attr($YITH_Woocompare_Frontend->add_compare_link());  // compare button
				}
				// YITH Quick View
				if( class_exists( 'YITH_WCQV_Frontend' ) ) {
					echo '<a href="#" class="button yith-wcqv-button" data-product_id="' . get_the_ID() . '">' . esc_attr__('Quick View','acadevo') . '</a>';
				}
				echo '</div><!-- .tm-hover-icons -->';	
			}
	...
	function tm_wc_single_wrap_start(){
		echo '<div class="themetechmount-single-product-details">';
	...
	function tm_wc_single_wrap_end(){
		echo '</div><!-- .themetechmount-single-product-details --> ';
	}

   
// add the filter 
add_filter( 'woocommerce_get_price_html_from_text', 'filter_woocommerce_get_price_html_from_text', 10, 1 ); 
