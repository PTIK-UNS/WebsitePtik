...
function themetechmount_shared_social_list(){
	...
	$sociallist = array(
		...
		'youtube'      => esc_attr__('YouTube', 'acadevo'),
		'flickr'       => esc_attr__('Flickr', 'acadevo'),
		'facebook'     => esc_attr__('Facebook', 'acadevo'),
		'linkedin'     => esc_attr__('LinkedIn', 'acadevo'),
		'gplus'        => esc_attr__('Google+', 'acadevo'),
		'yelp'         => esc_attr__('Yelp', 'acadevo'),
		'dribbble'     => esc_attr__('Dribbble', 'acadevo'),
		'pinterest'    => esc_attr__('Pinterest', 'acadevo'),
		'podcast'      => esc_attr__('Podcast', 'acadevo'),
		'instagram'    => esc_attr__('Instagram', 'acadevo'),
		'xing'         => esc_attr__('Xing', 'acadevo'),
		'vimeo'        => esc_attr__('Vimeo', 'acadevo'),
		'vk'           => esc_attr__('VK', 'acadevo'),
		'houzz'        => esc_attr__('Houzz', 'acadevo'),
		'issuu'        => esc_attr__('Issuu', 'acadevo'),
		'google-drive' => esc_attr__('Google Drive', 'acadevo'),
	);
...
function themetechmount_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="tm-acadevo-icon-arrow-left"></i> <span class="tm-hide tm-pagination-text tm-pagination-text-prev">' . esc_attr__( 'Previous page', 'acadevo' ) . '</span>',
		'next_text' => '<span class="tm-hide tm-pagination-text tm-pagination-text-next">' . esc_attr__( 'Next page', 'acadevo' ) . '</span> <i class="tm-acadevo-icon-arrow-right"></i>',
	) );
...
function themetechmount_get_option( $option, $inner_option='' ){
	...
	if( !is_array($acadevo_theme_options) ){
		
		if( is_multisite() ){
			$acadevo_theme_options = get_site_option('acadevo_theme_options');
		} else {
	...
	if( isset($acadevo_theme_options[$option]) ){
		$return = $acadevo_theme_options[$option];
	} else {
...
function themetechmount_get_all_registered_sidebars(){
	...
	$return = array( '' => esc_attr__('Default', 'acadevo') );
...
/**
 * Custom template tags for Acadevo
 *
...
if ( ! function_exists( 'acadevo_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Acadevo 1.0
 */
function acadevo_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation">
		<h2 class="screen-reader-text"><?php esc_attr_e( 'Comment navigation', 'acadevo' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_attr__( 'Older Comments', 'acadevo' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( esc_attr__( 'Newer Comments', 'acadevo' ) ) ) :
...
if ( ! function_exists( 'acadevo_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 *
 * @since Acadevo 1.0
 */
function acadevo_entry_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line posted-on"><i class="fa fa-calendar"></i> <span class="screen-reader-text tm-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
							esc_attr_x( 'Posted on', 'Used before publish date.', 'acadevo' ),
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="tm-meta-line byline"><i class="fa fa-user"></i> <span class="author vcard"><span class="screen-reader-text tm-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'acadevo' ),
							$author
						);
					}
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="fa fa-comment-o"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( '0', 'acadevo' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line cat-links"><i class="fa fa-tag"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'acadevo' ),
							$categories_list
						);
					}
				...
				case 'tag':
					$tags_list = get_the_tag_list( '', esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'acadevo' ) );
					if ( !empty($tags_list) ) {
						if( $tag_link!=true ){
						...
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="fa fa-tag"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'acadevo' ),
							$tags_list
						);
					}
 ...
 */
if( ! function_exists('acadevo_categorized_blog') ){
function acadevo_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'acadevo_categories' ) ) ) {
		...
		set_transient( 'acadevo_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so acadevo_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so acadevo_categorized_blog should return false.
		return false;
	}
}
}
...
function themetechmount_blogbox_comment_count( $echo = false ) {
	...
	if( comments_open() ){
		...
		$return  .= '<div class="themetechmount-blogbox-comment"><i class="tm-acadevo-icon-comment"></i> '. $comments .'</div>';
	}
	return $return;
}
}
...
function themetechmount_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		$library_kw_acadevo = ( !empty($data['library_kw_acadevo']) ) ? $data['library_kw_acadevo'] : '';
		...
		if( !empty(${'library_'.$library}) && ${'library_'.$library}!='empty' ){
			...
			$return = do_shortcode('[tm-icon type="'.$library.'" icon_kw_acadevo="'.$library_kw_acadevo.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
...
function themetechmount_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Next', 'acadevo' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_attr__( 'Next post:', 'acadevo' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Previous', 'acadevo' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_attr__( 'Previous post:', 'acadevo' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
		) );
...
function themetechmount_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_attr__('%s ', 'acadevo'), $val['pf_details_line_title'] ); }
...
function themetechmount_portfoliobox_media_link( $default_icon='tm-acadevo-icon-search', $video_icon='tm-acadevo-icon-video', $audio_icon='tm-acadevo-icon-music-alt', $slider_icon='tm-acadevo-icon-gallery-1' ){

	$icon_code='<i class="tm-acadevo-icon-plus-1"></i>';
...
function themetechmount_gallerystyle_portfoliobox_media_link( $default_icon='tm-acadevo-icon-search', $video_icon='tm-acadevo-icon-video', $audio_icon='tm-acadevo-icon-music-alt', $slider_icon='tm-acadevo-icon-gallery-1' ){

	$icon_code='<i class="tm-acadevo-icon-search"></i>';
	if( !empty($default_icon) ){
		$icon_code='<i class="' . $default_icon . '"></i>';
	}
...
function themetechmount_get_post_format_icon( $post_id='' ){
	...
	if( $post_type=='post' ){
		...
		$return = '<i class="tm-acadevo-icon-' . $icon_class . '"></i>';
...
function themetechmount_get_boxes( $cpt='blog', $vars=array() ){
	...
	$allword  = (!empty($vars['allword'])) ? $vars['allword'] : esc_attr__('All', 'acadevo');
		...
		while ( $posts->have_posts() ) {
			...
			if( $cpt == 'client' ){
				...
				if( $add_link == 'yes' && themetechmount_client_single_link()!='' ){
					$client_wrap_start .= '<a class="tm-client-logo-link" target="_blank" href="' . themetechmount_client_single_link() . '">';
...
function themetechmount_global_portfolio_template_list( $for_vc=false ){
	$return = array(	
			array(
				'label'	=> esc_attr__('Content overlay on image view','acadevo'),
				'value'	=> 'overlay',
 ...
 *  Global function - This will return array of Portfolio templates
 */
if( !function_exists('themetechmount_global_courses_template_list') ){
function themetechmount_global_courses_template_list( $for_vc=false ){
	$return = array(	
			array(
				'label'	=> esc_attr__('Style One','acadevo'),
				'value'	=> 'top-image',
				'thumb'	=> get_template_directory_uri() . '/inc/images/courses-style1.png',
			),
	);	
	if( $for_vc==true ){ // for vc
		$return = $return;
...
function themetechmount_global_blog_template_list( $for_vc=false ){
	...
	$return = array(	
				array(
					'label'	=> esc_attr__('Top image and bottom content','acadevo'),
					'value'	=> 'top-image',
					'thumb'	=> get_template_directory_uri() . '/inc/images/blogbox-style-one.png',
				),
				array(
					'label'	=> esc_attr__('Top image Style 2','acadevo'),
					'value'	=> 'top-image-style2',
					'thumb'	=> get_template_directory_uri() . '/inc/images/blogbox-style-three.png',
				),
				array(
					'label'	=> esc_attr__('Left image and right content','acadevo'),
					'value'	=> 'left-image',
...
function themetechmount_global_team_member_template_list( $for_vc=false ){
	...
	$return = array(	
			array(
				'label'	=> esc_attr__('Team Member Style 1','acadevo'),
				'value'	=> 'overlay',
				'thumb'	=> get_template_directory_uri() . '/inc/images/teambox-style1.png',
			),
			array(
				'label'	=> esc_attr__('Team Member Style 2','acadevo'),
				'value'	=> 'style2',
...
function themetechmount_global_testimonial_template_list( $for_vc=false ){
		$return = array(	
				array(
					'label'	=> esc_attr__('Testimonial Style 3','acadevo'),
					'value'	=> 'style-2',
					'thumb'	=> get_template_directory_uri() . '/inc/images/testimonial-style1.png',
				),
				array(
					'label'	=> esc_attr__('Testimonial Style 2','acadevo'),
					'value'	=> 'style-3',
					'thumb'	=> get_template_directory_uri() . '/inc/images/testimonial-style3.png',
				),
				array(
					'label'	=> esc_attr__('Testimonial Style 1','acadevo'),
					'value'	=> 'style-1',
...
/**
 *  Learnpress Course Box Function
 */
if ( !function_exists( 'themetechmount_coursebox_description' ) ){
function themetechmount_coursebox_description($post_id=''){
...
/**
 *   ThemetechMount Course Price
*/

if ( !function_exists( 'themetechmount_get_course_price' ) ){
function themetechmount_get_course_price( $tm_course ){
	$free_course_class = '';
	$tm_course = LP_Global::course();
	
	$tm_regular_price  = $tm_course->get_origin_price_html();
	$tm_last_price    = $tm_course->get_price_html();
	if($tm_last_price == 'Free') {
		$free_course_class="tm-free-course";
	}
	if ( $tm_course->has_sale_price() ) {
			$tm_course_price = sprintf( '<span class="tm-lp-price"><del>%2$s</del><ins>%1$s</ins></span>', $tm_last_price, $tm_regular_price );
	}
	else {
		$tm_course_price = sprintf( '<span class="tm-lp-price '. $free_course_class .'"><ins>%1$s</ins></span>', $tm_last_price );
	}
	return $tm_course_price;
}
}
...
function themetechmount_blogbox_readmore_text(){
	...
	$return             = esc_attr__('Read More', 'acadevo');
...
function themetechmount_entry_icon() {
	...
	$iconCode .= '<i class="tm-acadevo-icon-'.$icon.'"></i>';
...
if( !function_exists('themetechmount_get_instructorquery_args') ){
function themetechmount_get_instructorquery_args( $cptname='blog', $vars=array() ){
	
	$show     	= (!empty($vars['show'])) ? $vars['show'] : '3' ;
	$orderby	= (!empty($vars['orderby'])) ? $vars['orderby'] : 'id' ;
	$order		= (!empty($vars['order'])) ? $vars['order'] : 'DESC' ;
...
function themetechmount_blogbox_single_meta( $option='date', $args=array() ) {
	...
	switch($option){
		...
		case 'category' :
			...
			$categories_list = get_the_category_list( esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'acadevo' ) );
			if ( !empty($categories_list) ) {
...
function themetechmount_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if( is_home() ){
			...
			$title = esc_attr__( 'Blog', 'acadevo' );  // Setting for Titlebar title
		}
		...
		$adv_tbar_catarc = ( !empty($adv_tbar_catarc) ) ? esc_attr($adv_tbar_catarc.' %s') : esc_attr__('Category Archives: %s', 'acadevo') ;
		$title = sprintf(
			$adv_tbar_catarc,
		...
		$adv_tbar_tagarc = ( !empty( $adv_tbar_tagarc ) ) ? esc_attr($adv_tbar_tagarc.' %s') : esc_attr__('Tag Archives: %s','acadevo') ;
		$title = sprintf(
			$adv_tbar_tagarc,
		...
		if( is_tax('tm_team_group') || is_tax('tm_portfolio_category') ){
			...
			global $wp_query;
			$adv_tbar_postclassified = ( !empty($adv_tbar_postclassified) ) ? esc_attr($adv_tbar_postclassified.' %s') : esc_attr__('Posts classified under: %s', 'acadevo') ;
		...
		$adv_tbar_authorarc = ( !empty( $adv_tbar_authorarc ) ) ? esc_attr($adv_tbar_authorarc.' %s') : esc_attr__('Author Archives: %s', 'acadevo');
		...
		$title    = sprintf( esc_attr__( 'Search Results for %s', 'acadevo' ), '<span class="tm-titlebar-heading tm-tbar-search-title">' . get_search_query() . '</span>' );
	
	} else if( is_404() ){ // 404
		$error404_big_text = themetechmount_get_option('error404_big_text');
		$title             = sprintf( esc_attr__( '%s ', 'acadevo' ), $error404_big_text );
		...
		if( function_exists('tribe_is_month') && tribe_is_month() && !is_tax() ) { // The Main Calendar Page
			...
			global $wp_query;
			$title = esc_attr__( 'Events on: ', 'acadevo' ). date('F j, Y', strtotime($wp_query->query_vars['eventDate']));
		...
		// BBPress section
			...
			$title = esc_attr__( 'Forum', 'acadevo' );
		} else if( is_post_type_archive() ){
			$title = post_type_archive_title('', false);
		} else if ( is_day() ){
			$title = sprintf( esc_attr__( 'Daily Archives: %s', 'acadevo' ), '<span>' . get_the_date() . '</span>' );
		} elseif( is_month() ){
			$title = sprintf( esc_attr__( 'Monthly Archives: %s', 'acadevo' ), '<span>' . get_the_date( esc_attr_x( 'F Y', 'monthly archives date format', 'acadevo' ) ) . '</span>' );
		} elseif( is_year() ){
			$title = sprintf( esc_attr__( 'Yearly Archives: %s', 'acadevo' ), '<span>' . get_the_date( esc_attr_x( 'Y', 'yearly archives date format', 'acadevo' ) ) . '</span>' );
		} else {
			if( function_exists('is_bbpress') && is_bbpress() ) {
				$title = esc_attr__( 'Forum', 'acadevo' );
			} else {
				$title = esc_attr__( 'Archives', 'acadevo' );
			}
		};
...
function themetechmount_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="tm-header-icon tm-header-wc-cart-link"><a href="' . $cart_url . '"><i class=" tm-acadevo-icon-basket"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="tm-header-icon tm-header-search-link"><a href="#"><i class="tm-acadevo-icon-search"></i></a><div class="tm-search-overlay"><form method="get" class="tm-site-searchform" action=" ' . esc_url( home_url() ) . '"> <div class="w-search-form-h"><div class="w-search-form-row"><div class="w-search-input"><input type="search" class="field searchform-s" name="s" placeholder="' . esc_attr($search_input) . '" /><button type="submit"><span class="tm-acadevo-icon-search"></span></button></div></div></div></form></div></div>';
	}
	...
	if( themetechmount_fbar_show()==true){
		...
		<span class="themetechmount-fbar-btn ' . themetechmount_sanitize_html_classes(themetechmount_fbar_btn_classes()) . '">
			<a href="#" class="themetechmount-fbar-btn-link">
				...
				<span class="tm-hide">' . esc_attr__('Open', 'acadevo') . '</span>
			</a>
		</span>';
	}
...
function themetechmount_footer_row_class( $row='first' ){
	...
	// BG color
	if( !empty($acadevo_theme_options[$row.'_footer_bg_color']) ){
		$class .= ' tm-bg tm-bgcolor-'.sanitize_html_class($acadevo_theme_options[$row.'_footer_bg_color']);
	}
	
	// Text color
	if( !empty($acadevo_theme_options[$row.'_footer_text_color']) ){
		$class .= ' tm-textcolor-'.sanitize_html_class($acadevo_theme_options[$row.'_footer_text_color']);
	}
	
	// If bg image is there
	if( !empty($acadevo_theme_options[$row.'_footer_bg_all']['image']) ){
	...
	// border type
	if( !empty($acadevo_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' tm-bordertype-'.sanitize_html_class($acadevo_theme_options[$row.'_footer_bordertype']);
	}
	
	//widget border color
	if( !empty($acadevo_theme_options[$row.'_footer_widget_bordercolor']) ){
		$class .= ' tm-widgetbordercolor-'.sanitize_html_class($acadevo_theme_options[$row.'_footer_widget_bordercolor']);
	}
 ...
 */
if( !function_exists('acadevo_addhttp') ){
	function acadevo_addhttp($url){
		if (!preg_match("~^(?:f|ht)tps?://~i", $url)){
			$url = "http://" . $url;
		}
		return $url;
...
function themetechmount_testimonial_featured_image($size='thumbnail'){
	...
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="themetechmount-icon-box"><i class="demo-icon tm-acadevo-icon-quote-left"></i></span>';
...
function themetechmount_star_ratting(){
	...
	for ($i = 1; $i <= 5; $i++) {
		if( $star_ratings >= $i ){
			$return .= ' <i class="tm-acadevo-icon-star-1 tm-active"></i> ';
		} else {
			$return .= ' <i class="tm-acadevo-icon-star-empty-1"></i> ';
		}
	}
	return $return;
}
...
function themetechmount_revslider_array( $countonly=false ) {
	...
	if( $countonly==true ){
		...
		if( count($sliders)==0 ){
			$sliders[''] = esc_attr__('(No Slider Found)', 'acadevo');
		}
		return $sliders;
	}
}
	...
	if ( function_exists('lsSliders') ) {
		...
		if( $countonly == true ){
			...
			if( count($slider_names) == 0 ){
				$slider_names[''] = esc_attr__('(No Slider Found)', 'acadevo');
			}
			return $slider_names;
		}
...
function themetechmount_get_sidebar_info( $return_total_count = false ){
	...
	if( is_front_page() && !is_page() ){
		...
		if( is_singular('lp_course') ){
			$sidebar_lpcourse = themetechmount_get_option('sidebar_lpcourse');
			$sidebar = ( !empty( $sidebar_lpcourse ) ) ? esc_attr( $sidebar_lpcourse ) : 'no' ; // Global settings
			$count_widgets = themetechmount_count_sidebar( 'lpcourse', $sidebar );
		}
	}
	...
	// Learnpress Course (The Learnpress plugin)
...
function themetechmount_single_get_sidebar_value(){
	...
	$sidebar = $acadevo_theme_options['sidebar_post'];
	...
	if( is_page() || is_singular() ){
		...
		if( !empty($acadevo_theme_options['sidebar_'.$cpt]) ){
			$sidebar = $acadevo_theme_options['sidebar_'.$cpt];
		}
	...
	if( is_search() ){
		$sidebar = $acadevo_theme_options['sidebar_search'];
	}
...
function themetechmount_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'acadevo') . '';
...
function themetechmount_event_readmore(){ 
	$return   = '';
	$readMore = esc_attr__('View Details', 'acadevo') . ' <i class="kwicon-fa-angle-right"></i>';
	$return = '<a href="'.get_permalink().'" class="tm-vc_general tm-vc_btn3 tm-vc_btn3-size-md tm-vc_btn3-shape-square tm-vc_btn3-style-flat tm-vc_btn3-weight-no tm-vc_btn3-color-skincolor">'.$readMore.'</a>';
	return $return;
}
}
...
function themetechmount_comment_row_template($comment, $args, $depth){
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			 <em class="comment-awaiting-moderation"><?php esc_attr_e( 'Your comment is awaiting moderation.', 'acadevo' ); ?></em>
			  <br />
		<?php endif; ?>
		<div class="comment-box">
			<div class="comment-meta commentmetadata">
				<?php printf( '<cite class="tm-comment-owner fn">%s</cite> <span class="says">' . esc_attr__('says:', 'acadevo' ) . '</span>', get_comment_author_link() ); ?>
				
				<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
				<?php
				/* translators: 1: date, 2: time */
				printf( esc_attr__( '%1$s at %2$s', 'acadevo' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_attr__( '(Edit)', 'acadevo' ), '  ', '' );
				?>
			</div>			
...
function themetechmount_author_social_links(){
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a href="'. $social_link .'" target="_blank"><i class="tm-acadevo-icon-'. $social_class .'"></i><span class="tm-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
...
function themetechmount_search_results_box_title( $post_type='post' ){
	...
	$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'" class="label label-default"><i class="tm-acadevo-icon-angle-left"></i> '.esc_attr__('Back to results','acadevo').'</a></small>';
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_attr__('View more','acadevo').'</a></small>';
	}
...
function themetechmount_search_form(){
	...
	$cptList = array(
		'any'            => esc_attr__('All selections', 'acadevo'),
		'page'           => esc_attr__('Pages', 'acadevo'),
		'post'           => esc_attr__('Blog posts', 'acadevo'),
	);
	...
	<div class="tm-sresult-form-wrapper">
					...
					<div class="tm-sresult-form-bottom-w">
						...
						<div class="tm-search-main-box clearfix">
                            <div class="tm-search-text"><strong>' . esc_attr__('Search in:','acadevo') . '</strong></div>
							<div class="tm-search-select-box">
								' . $dropdown . '
                              <div class="tm-sresult-form-sbtbtn-wrapper">
								<input class="tm-sresult-form-sbtbtn" type="submit" value="' . esc_attr__('Search now','acadevo') . '" />
							  </div>
						  </div>
...
function themetechmount_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = esc_attr_x( '%1$s on %2$s', '1: post format name. 2: date', 'acadevo' );
	} else {
		$format_prefix = '%2$s';
	}
	...
	$date = '<div class="themetechmount-post-date-wrapper">';
		$date .= sprintf( '<div class="themetechmount-entry-date-wrapper"><span class="themetechmount-entry-date"><time class="entry-date" datetime="%1$s" >%2$s<span class="entry-month entry-year">%3$s</span></time></span></div>',
			get_the_date( 'c' ),
			get_the_date( 'j' ),
			get_the_date( 'M' ),
...
if ( ! function_exists( 'acadevo_blogstyle2_meta' ) ) :

function acadevo_blogstyle2_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="tm-meta-line byline"><i class="tm-acadevo-icon-user-1"></i> <span class="author vcard"><span class="screen-reader-text tm-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'acadevo' ),
							$author
						);
					}
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="tm-acadevo-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( 'Leave a comment', 'acadevo' ) );
						$return .= ob_get_contents();
						ob_end_clean();
						$return .=  '</span>';
					}
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line cat-links"><i class="tm-acadevo-icon-folder"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'acadevo' ),
							$categories_list
						);
					}
...
if ( ! function_exists( 'acadevo_posttime_meta' ) ) :
function acadevo_posttime_meta( $metafor="blogbox" ) {
...
function themetechmount_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	...
	$return = '';
	if( !empty($acadevo_theme_options['social_icons_list']) ){
		foreach( $acadevo_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="tm-social-' . $social_id . ' tm-socialbox-i-wrapper"><a class="tm-socialbox-icon-link tm-socialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="tm-acadevo-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	...
	foreach( $socialArray as $key=>$value ){
		...
		if( $key == 'rss' ){
			if( isset($acadevo_theme_options['rss']) && $acadevo_theme_options['rss']=='1' ){
			...
			if( isset($acadevo_theme_options[$key]) && trim($acadevo_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a target="_blank" href="'.esc_url($acadevo_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="tm-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
...
/********************** Team Search Form ************************/
if( !function_exists('themetechmount_team_search_form') ){
function themetechmount_team_search_form($title='', $form_desc='', $search='', $submit_btn='', $form_type=''){
	...
	if( is_array($termList) && count($termList)>0 ){
		...
		$dropDown .= '<div class="tm-wrap-cell tm-fbar-input"> <div class="search_field selectbox"> <i class="fa fa-tags"></i> <select name="team_group"> <option value="" class="select-empty">' . esc_attr__('All Sections', 'acadevo') . '</option>';
		...
		$noGroupClass = 'themetechmount-team-form-no-group';
		$inputClass   = 'tm-wrap-cell tm-fbar-input';
	}
	...
	// Search title
	$doctor_search_title = esc_attr__('Doctor Search:', 'acadevo');
	$doctor_search_title = esc_attr($title);
	
	if( !empty($doctor_search_title) ){
		$doctor_search_title = '<h2>'. $doctor_search_title .'</h2>';
	}
	...
	$tm_placeholder = esc_attr__('Search by name','acadevo');
	if(!empty($search)){
		$tm_placeholder = esc_attr($search);
	}
	
	//Search Button Text
	$submit_button = esc_attr__('Search' , 'acadevo');
	if(!empty($submit_btn)){
	...
	$return .= '<div class="team-search-form-w '. sanitize_html_class($formclass) .'"><div class="team-search-form-inner-w">
		<form method="get" class="team-search-form '. sanitize_html_class($noGroupClass) .'" action="'.esc_url( home_url( '/' ) ).'">
					...
					<div class="tm-wrap">
						...
						'. $text_before_form .'
						<div class="'.  themetechmount_sanitize_html_classes($inputClass) .'">
...
/**
 * get Boxes for Instructor
 */
if( !function_exists('themetechmount_get_instructorboxes') ){
function themetechmount_get_instructorboxes( $cpt='', $vars=array() ){

	global $instructor_post;
	global $tm_instshowshow;	
	$return            = '';
	
	$instructor_post    = (!empty($vars['posts']))   ? $vars['posts'] : '' ;
	$column   = (!empty($vars['column']))  ? $vars['column'] : 'three' ;
	$template = (!empty($vars['view']))    ? $vars['view'] : 'top-image' ;
	$boxview  = (!empty($vars['boxview'])) ? $vars['boxview'] : '';
	
	$tm_instshowshow  = (!empty($vars['show'])) ? $vars['show'] : '3' ;

	global $tm_instrucotr_id;
	global $tm_instructor_name;
	global $tm_instructor_desig;
	global $tm_instructor_desc;
		
	foreach ( $instructor_post->results as $instructor ) {		
			$tm_instrucotr_id = $instructor->ID;
			$tm_instructor_name = $instructor->display_name;
			$tm_instructor_desig = get_the_author_meta( 'tm_instructor_designation', $tm_instrucotr_id );
			$tm_instructor_desc = get_the_author_meta( 'description', $tm_instrucotr_id );
	...
	if( !empty($return) ){
		...
		';
				...
				// Pagination
		if( isset($vars['pagination']) && $vars['pagination']=='yes' && $vars['boxview']!='carousel' ){
			$return .= themetechmount_instructor_pagination( $instructor_post );
		}
...
/**
 *  Previous/next page navigation for instructor box.
 */
if( !function_exists('themetechmount_instructor_pagination') ){
function themetechmount_instructor_pagination( $wp_query_data=false ){
	...
	$show     	= $wp_query->role;
	$big     = 999999999; // need an unlikely integer
	$total = $wp_query->get_total();
	$max_num_pages = ceil( $total/$tm_instshowshow );
...
if( !function_exists('themetechmount_box_instructor_social_links') ){
function themetechmount_box_instructor_social_links($tm_instrucotr_id){
		
	$return = '';	
	$all_socials = array();
	
	$all_socials['twitter'] = get_the_author_meta( 'twitter', $tm_instrucotr_id );
	$all_socials['facebook'] = get_the_author_meta( 'facebook', $tm_instrucotr_id );
	$all_socials['linkedin'] = get_the_author_meta( 'linkedin', $tm_instrucotr_id );
	$all_socials['gplus']  	= get_the_author_meta( 'gplus', $tm_instrucotr_id );
	...
	if( !empty($return) ){
		$return = '<div class="tm-author-social-links-wrapper"><ul class="tm-team-social-links">' . $return . '</ul></div>';
	}
